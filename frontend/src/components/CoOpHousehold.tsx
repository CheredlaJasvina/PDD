...
export const CoOpHousehold: React.FC = () => {
  ...
  const fetchHousehold = async () => {
    try {
      const response = await fetch('https://srv-da5dcaqjobas73ebumk0.onrender.com/api/household');
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    if (!joinCode) return;
    try {
      const response = await fetch('https://srv-da5dcaqjobas73ebumk0.onrender.com/api/household/join', {
        method: 'POST',
  ...
  const handleAddChore = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://srv-da5dcaqjobas73ebumk0.onrender.com/api/household/chores', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleToggleChore = async (id: string) => {
    try {
      const response = await fetch(`https://srv-da5dcaqjobas73ebumk0.onrender.com/api/household/chores/${id}/toggle`, {
        method: 'PUT'
      });
      const data = await response.json();
      if (data.success) {
