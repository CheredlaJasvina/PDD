...
export const Recipes: React.FC<RecipesProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'suggestions' | 'liked'>('suggestions');
  ...
  const fetchRecipes = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://pdd-9fqv.onrender.com/api/recipes?servings=${preferences.servings}&mode=${preferences.audienceMode}`, {
  ...
  const activeRecipes = recipes.filter(r => {
    ...
    if (deletedRecipes.includes(r.title)) return false;
    return true;
  });
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem', marginBottom: '0.5rem' }}>
            <button className="btn-secondary" style={{ padding: '0.5rem 1rem', background: viewMode === 'suggestions' ? 'var(--color-fresh)' : 'transparent', color: viewMode === 'suggestions' ? '#000' : 'var(--text-main)', border: '1px solid var(--color-fresh)' }} onClick={() => setViewMode('suggestions')}>Suggestions</button>
            <button className="btn-secondary" style={{ padding: '0.5rem 1rem', background: viewMode === 'liked' ? 'var(--color-fresh)' : 'transparent', color: viewMode === 'liked' ? '#000' : 'var(--text-main)', border: '1px solid var(--color-fresh)' }} onClick={() => setViewMode('liked')}>Liked Recipes 💖</button>
          </div>
          ...
          {viewMode === 'liked' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              {likedRecipes.length === 0 ? (
                <div className="glass-card" style={{ textAlign: 'center', padding: '4rem', color: 'var(--text-muted)' }}>
                  <p style={{ fontSize: '1.2rem', marginBottom: '0.5rem' }}>No liked recipes yet.</p>
                  <p style={{ fontSize: '0.9rem' }}>Save some recipes to see them here.</p>
                </div>
              ) : (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid var(--glass-border)', paddingBottom: '1.25rem', marginBottom: '1.5rem' }}>
                      <div>
                        <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--color-warning)', fontWeight: 700, letterSpacing: '1px' }}>
                          💡 Matches: {recipe.primaryIngredient}
                        </span>
                      ...
                      <button className="btn-secondary" style={{ padding: '0.5rem 1rem', borderColor: 'var(--color-spoiled)', color: 'var(--color-spoiled)' }} onClick={() => setLikedRecipes(prev => prev.filter(r => r.title !== recipe.title))}>
                        Remove ❌
                      </button>
                    </div>
                    {/* Simplified view for Liked Recipes */}
                ...
                <div key={idx} className="glass-card" style={{ padding: '2rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', borderBottom: '1px solid var(--glass-border)', paddingBottom: '1.25rem', marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                      ...
                      <button className="btn-secondary" style={{ padding: '0.5rem 1rem', borderColor: 'var(--color-fresh)', color: 'var(--color-fresh)' }} onClick={() => {
                        if (!likedRecipes.find(r => r.title === recipe.title)) {
                          setLikedRecipes(prev => [...prev, recipe]);
                        }
                      }}>
                        Save 💖
                      </button>
                      <button className="btn-secondary" style={{ padding: '0.5rem 1rem', borderColor: 'var(--color-spoiled)', color: 'var(--color-spoiled)' }} onClick={() => setDeletedRecipes(prev => [...prev, recipe.title])}>
                        Delete 🗑️
                      </button>
                    </div>
                  </div>
